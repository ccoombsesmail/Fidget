...
const DropDownMenu = ({ currentUser, showMenu, logout, login, toggle, history, currentChannel }) => {
  ...
  const menuClasses = [classes.dropDown]
  if (showMenu) {
    ...
    currentUser ? (
      <ul className={menuClasses.join(' ')} onClick={e => e.stopPropagation()}>
        <div className={classes.topDiv}> 
          <div onClick={() => handleClick('dashboard')}  className={classes.userIconWrapper}> 
            <img alt="" className={classes.userIcon} src={currentChannel ? currentChannel.logoUrl : null} />
          </div>
          <h5>{currentUser.username}</h5>
        </div>
        <hr />
        <li onClick = {() => handleClick('showChannel')}>
          <FontAwesomeIcon className={classes.userIconList} icon={faVideo} /> 
          <span> Channel </span> 
        </li>
        <li onClick = {() => handleClick('dashboard')}>
          <FontAwesomeIcon className={classes.userIconList} icon={faCog} />
          <span> Dashboard </span> 
        </li>
        <li onClick={() => handleClick('logout')}>
          <FontAwesomeIcon className={classes.userIconList} icon={faSignOutAlt} />
          <span> Sign Out </span> 
        </li>
      </ul>
    ) : (
      <ul style = {{height: '150px'}} className={menuClasses.join(' ')} onClick={e => e.stopPropagation()}>
        <li onClick={() => handleClick('login')}>
          <FontAwesomeIcon className={classes.userIconList} icon={faSignInAlt} /> 
          <span> Sign In </span> 
        </li>
      </ul>
    )
